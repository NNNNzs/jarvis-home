# Just A Rather Very Intelligent System (JARVIS) - Home

一个基于 **Node.js + TypeScript** 的多智能体智能家居控制系统。

## 🎯 项目目标

构建一个基于 LLM (LangChain/LangGraph) 的多智能体协同系统，作为 Home Assistant 的"智能前置大脑"，实现：

- 🗣️ **自然语言控制**: 通过一句话控制多个智能设备
- 🧠 **意图理解**: 自动识别用户真实意图
- 📊 **状态感知**: 实时获取家庭设备状态
- 🔄 **流程缓存**: 避免重复推理，提高响应速度
- 🔌 **设备集成**: 通过 Home Assistant 统一管理所有设备

## 🚀 核心场景

**用户输入**: "我要洗澡了"  
**系统响应**:

- ✅ 开启燃气热水器预热
- ✅ 打开浴霸
- ✅ 打开浴室灯
- ✅ (可选) 播放音乐、调节水温

## 📐 架构概览

**详细流程图**: 查看 [docs/system-flowchart.drawio](./docs/system-flowchart.drawio) (使用 [draw.io](https://app.diagrams.net/) 打开)

```
┌──────────────┐     ┌─────────────────────┐
│  语音/HTTP入口 │ ──> │  LLM Orchestrator   │
└──────────────┘     │   (LangGraph)       │
                     └──────────┬──────────┘
                                │
           ┌────────────────────┼────────────────────┐
           ▼                    ▼                    ▼
    ┌──────────┐        ┌──────────┐        ┌──────────┐
    │ Intent   │        │ Context  │        │ Planner  │
    │ Agent    │        │ Agent    │        │ Agent    │
    └──────────┘        └──────────┘        └──────────┘
                                │
                                ▼
                     ┌──────────────────┐
                     │ Home Assistant   │
                     │   (设备状态&执行) │
                     └──────────────────┘
```

## 🛠️ 技术栈

- **语言**: TypeScript / Node.js
- **LLM 框架**: LangChain + LangGraph
- **API 框架**: Express
- **状态管理**: Home Assistant
- **验证**: Zod
- **HTTP 客户端**: Axios
- **缓存**: Redis (可选) / 内存

## 📁 项目结构

```
JustARatherVeryIntelligentSystem/
├── src/
│   ├── agents/           # 智能体模块
│   │   ├── intent.ts     # 意图识别
│   │   ├── context.ts    # 状态感知
│   │   └── planner.ts    # 行为规划
│   ├── services/         # 服务层
│   │   ├── homeassistant.ts  # HA API
│   │   ├── llm.ts        # LLM 调用
│   │   └── cache.ts      # 缓存服务
│   ├── api/              # HTTP 接口
│   │   ├── routes.ts
│   │   └── middleware.ts
│   ├── types/            # 类型定义
│   │   └── index.ts
│   └── app.ts            # 应用入口
├── tests/                # 测试
├── .env                  # 环境变量 (需创建)
├── tsconfig.json         # TypeScript 配置
└── package.json          # 依赖管理
```

## 🚀 快速开始

### 1. 环境准备

```bash
# 安装依赖
pnpm install

# 创建环境变量文件
# 复制下面的配置到 .env 文件
```

### 2. 配置环境变量

在项目根目录创建 `.env` 文件。

**最小配置** (仅测试):

```bash
# LLM 服务（必需，二选一）
OPENAI_API_KEY=sk-your-api-key-here
# 或
ANTHROPIC_API_KEY=sk-ant-your-key-here
```

**完整配置** (生产环境):

```bash
# LLM 服务
ANTHROPIC_API_KEY=sk-ant-your-key-here
ANTHROPIC_BASE_URL=  # 可选，自定义接口地址

# 向量模型服务（独立接口，可选）
EMBEDDING_API_KEY=your-embedding-key-here
EMBEDDING_BASE_URL=https://api.siliconflow.cn/v1
EMBEDDING_MODEL=BAAI/bge-large-zh-v1.5

# Home Assistant（可选）
HOME_ASSISTANT_URL=http://homeassistant.local:8123
HOME_ASSISTANT_TOKEN=your_ha_token_here

# 其他配置
PORT=3000
NODE_ENV=development
```

**详细配置说明**: 查看 [docs/env-variables.md](./docs/env-variables.md)

**重要**: LLM 和向量模型可以使用不同的接口地址，详见环境变量文档。

### 3. 配置 Home Assistant

1. 登录 Home Assistant
2. 进入 "用户配置" → "安全"
3. 创建 "长生命周期访问令牌"
4. 复制到 `.env` 文件的 `HOME_ASSISTANT_TOKEN`

### 3. 配置 LLM

在 `.env` 文件中填入你的 OpenAI API Key (或其他兼容的 LLM 服务)

### 4. 运行

```bash
# 开发模式
pnpm dev

# 生产模式
pnpm build
pnpm start
```

### 5. 测试

```bash
# 发送测试请求
curl -X POST http://localhost:3000/api/intent \
  -H "Content-Type: application/json" \
  -d '{"message": "我要洗澡了"}'
```

## 📋 API 端点

### POST /api/intent

处理用户意图输入

**请求体**:

```json
{
  "message": "我要洗澡了"
}
```

**响应**:

```json
{
  "success": true,
  "intent": "bath_prepare",
  "plan": [...],
  "execution": [...]
}
```

### GET /api/status

系统状态检查

## 🎯 开发路线图

### Phase 1: MVP (1 周)

- ✅ 基础项目结构
- ✅ Home Assistant 接入
- ✅ 单意图支持 ("我要洗澡了")
- ⏳ 基础流程执行

### Phase 2: 智能化 (2 周)

- ⏳ 多 Agent 协作
- ⏳ 上下文感知
- ⏳ 流程缓存机制

### Phase 3: 高级功能

- ⏳ 多意图支持
- ⏳ 学习型偏好
- ⏳ 小爱音箱集成

## ⚠️ 重要说明

### 小爱音箱接入

由于小爱音箱没有官方开放 API，推荐方案：

- **米家场景 → Webhook**: 在米家 App 中创建场景，触发 HTTP 请求到本系统
- 固定话术触发，如："我要洗澡了"

### 安全限制

- 白名单设备控制
- 高风险操作需要确认
- 所有操作可追溯、可回滚

## 🔧 开发配置

### Cursor Rules

项目已配置 `.cursor/rules/` 目录，包含：

- 代码规范
- 提交信息规范
- 架构决策记录

### 环境变量

- 创建 `.env` 文件 (参考 [docs/env-variables.md](./docs/env-variables.md))
- **重要**: `.env` 文件已加入 `.gitignore`，不要提交到版本控制

## 🤝 贡献

这是一个"工程味很正"的项目，欢迎：

- 提交 Issue
- 提交 PR
- 分享使用场景

## 📝 需求文档

详细的需求文档和设计文档请参考项目内的讨论记录，包含了完整的：

- 架构设计
- 技术选型依据
- 实施路线图
- 风险评估

## 📄 License

MIT
